  ...
  cursor: z
    .preprocess(
      val => {
        ...
        if (val && typeof val === 'object' && !('sortField' in val)) {
          const legacyCursor = val as { updatedAt: number; id: string }
          return { sortField: 'updated' as const, ...legacyCursor }
        }
        return val
      },
      z.discriminatedUnion('sortField', [
        z.object({
